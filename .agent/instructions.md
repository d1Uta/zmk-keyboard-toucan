...
- 作業中に実行したすべてのコマンドを、その**実行意図（Why）とともに詳細に記録**した報告書（Artifact）を都度作成すること。これは Antigravity の基本方針である。
- 同一スレッドで作業を継続する場合、過去の報告内容を上書きせず、追記または新規ファイル作成によって履歴を保持し、全作業の透明性を確保すること。
...
- **エラーの発生**: コマンドが失敗した場合や、期待しないエラー出力があった場合。原因の分析結果および、それに基づく**再発防止策**を速やかに報告すること。
- **ナレッジへの追記**: エラーを解決した際は、即座にその原因、解決策、および再発防止策を `knowledge/troubleshooting.md` や関連する **`Skills`** に追記すること（セクション 9 参照）。
...
- **エラーの解決**: 新たなエラーに遭遇し、それを解決した場合は、その原因、解決策、および**再発防止策**を `knowledge/troubleshooting.md` や関連する **`Skills`** の適切なセクションに追記する。
