...
### CI エラー：409 Conflict (Artifact already exists)
- **事象**: `Failed to CreateArtifact: (409) Conflict: an artifact with this name already exists...`
- **原因**: 複数のマトリックスジョブが同じ Shield/Board 名でビルドを行う際、デフォルトのアーティファクト名が重複してアップロード先で衝突するため。
- **解決策**: 各ディレクトリの `build.yaml` において、`include` セクションの各ターゲットに `artifact-name` プロパティを追加し、バリアント名を含めた一意の名前（例: `toucan_left_mac_onishi`）を指定する。
- **再発防止策**: 複数バリアントを同時にビルドするリポジトリ構成では、デフォルト設定に頼らず、必ず `artifact-name` を明示的に個別に定義する。
