...
### CI エラー：Merge Artifacts 失敗 (Race Condition) - 最終結論
- **事象**: 一部のマージジョブが「成果物が見つからない」あるいは衝突エラーで失敗する。
- **原因**: 
    1. **並列マージの不整合**: メインの `build.yml` でマトリックスを使用し、その中で外部ワークフローを個別に呼び出すと、複数の独立したマージジョブが同時に「全成果物をマージして削除」しようとする。
    2. **成果物の強奪**: 最初に終わったジョブが全成果物を削除してしまうため、後続のジョブが処理対象を見失う（椅子取りゲーム状態）。
- **根本解決策**: 
    1. **ビルドの統合**: `build.yml` 側のマトリックスを廃止し、ZMK の再利用可能ワークフローを**一度だけ**呼び出すように変更する。
    2. **統合管理ファイルの作成**: ルートに `build.yaml` を配置し、そこに全 12 ターゲット（OS/配列別）を記述する。
    3. **ルートマニフェストの定義**: `west.yml` をルートに作成し、`self: path: .` と指定することで、統合ビルド環境から各フォルダの設定を正しく参照可能にする。
- **再発防止策**: 並列で動作する外部ワークフローが「共通の成果物パターン（artifact-*）」を操作する場合、呼び出し側で個別にマトリックスを組まず、ワークフロー内部のマトリックス機能に一括して任せる設計を優先する。
