...
- **原因**: 
    ...
    2. **統合ビルドの限界**: これを解消するために「単一行列（1ジョブ）」に統合を試みたが、ディレクトリ参照（`ZMK_CONFIG`）のパス解決が複雑になりビルドエラーが発生した。
- **最終解決策 (決定版)**:
    1. **ジョブ単位での成果物分離**: `build.yml` で **`archive_name: toucan_${{ matrix.variant }}`** を指定し、各ジョブが書き出す ZIP ファイル自体の名前を完全に分離する。
    2. **物理的な競合排除**: マージジョブが互いのファイルを削除し合う余地をなくす。
    3. **サイレント・フォールバックの防止**: ルートの `build.yaml` や `west.yml` を削除し、個別フォルダの設定を明示参照させる。
- **再発防止策**: 複雑なマトリックスビルドでは、成果物の「中身の名前」だけでなく「成果物コンテナ（ZIP）自体の名前」も一意に設計し、並列ジョブが完全に独立して動作できるようにする。
