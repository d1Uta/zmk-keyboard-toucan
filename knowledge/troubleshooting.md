...
### コマンド連結 (Command Chaining)
- **事象**: `git add . && git commit ...` や `gh pr create ... && ...` などのコマンドが `トークン '&&' は、このバージョンでは有効なステートメント区切りではありません。` (The token '&&' is not a valid statement separator) で失敗する。
- **原因**: 動作環境である Windows PowerShell は、Bash や CMD と異なり `&&` 演算子を標準でサポートしていない（PowerShell 7 以降ではサポートされる場合があるが、環境依存を避けるため使用しない）。
- **解決策**:
    - **セミコロン (`;`) を使用する**: `git add .; git commit ...`
    - **改行して別々に実行する**: `run_command` を複数回呼び出すか、スクリプトブロック内で改行する。
- **特に注意**: `gh` コマンド等を含む長いワンライナーを提示する際は、必ず `;` で連結するか、確実に PowerShell 互換であることを確認する。
