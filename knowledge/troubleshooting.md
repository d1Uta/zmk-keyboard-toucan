...
### CI エラー：lexer: invalid input text (yq quoting)
- **事象**: `lexer: invalid input text "zmk_config/build..."` というエラーでビルドマトリックスの取得に失敗する。
- **原因**: `build_matrix_path` に `load()` 関数を使用する際、内部のクォート（`"`）がワークフロー側の外部クォートと衝突してシェルコマンドが破壊されたため。
- **解決策**: `build_matrix_path: "load('zmk_config/build.yaml')"` のように、内部にシングルクォートを使用することで、シェルによる解釈の衝突を回避する。
- **再発防止策**: GitHub Actions の `inputs` を介して複雑な yq クエリ（特にファイルパスを含むもの）を渡す際は、クォートの入れ子構造に細心の注意を払う。
