...
- **原因**: 
    ...
    2. **並列ジョブの弊害**: これを解消するために「ジョブ単位の成果物分離 (`toucan_mac_onishi.zip` 等)」を試みたが、構成の理解不足により一時的に統合ビルドへと戻してしまった。
- **最終解決策 (決定版) - 独立並列構成**:
    1. **ジョブ単位での成果物分離**: `build.yml` で **`archive_name: toucan_${{ matrix.variant }}`** を指定し、各ジョブが書き出す ZIP ファイル自体の名前を完全に分離する。これによりマージの競合を物理的に不可能にする。
    2. **config_path: . の禁止**: `config_path: .` を指定すると、内部的に `mkdir /tmp/zmk-config/.` が実行され、「File exists」エラーで失敗する（ZMK ワークフローの制限）。
    3. **個別ディレクトリの明示参照**: `config_path: ${{ matrix.path }}` とすることで上記のエラーを回避し、かつ各フォルダの `build.yaml` と `west.yml` を正しく読み込ませる。
- **再発防止策**: 外部ワークフローをマトリックスで使用する際は、ルートディレクトリ（`.`）を直接設定パスに指定することを避け、常に固有の名前を持つサブディレクトリを割り当てることで、ファイルシステムの衝突や暗黙の制限を回避する設計を徹底する。
